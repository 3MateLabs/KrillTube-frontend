...
export function isClock(type: string): boolean { type = compressIotaType(type); return type === `${PKG_V5}::clock::Clock`; }
...
export class Clock implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V5}::clock::Clock`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Clock.$typeName; readonly $fullTypeName: `${typeof PKG_V5}::clock::Clock`; readonly $typeArgs: []; readonly $isPhantom = Clock.$isPhantom;

 readonly id: ToField<UID>; readonly timestampMs: ToField<"u64">

 private constructor(typeArgs: [], fields: ClockFields, ) { this.$fullTypeName = composeIotaType( Clock.$typeName, ...typeArgs ) as `${typeof PKG_V5}::clock::Clock`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.timestampMs = fields.timestampMs; }

 static reified( ): ClockReified { return { typeName: Clock.$typeName, fullTypeName: composeIotaType( Clock.$typeName, ...[] ) as `${typeof PKG_V5}::clock::Clock`, typeArgs: [ ] as [], isPhantom: Clock.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Clock.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Clock.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Clock.fromBcs( data, ), bcs: Clock.bcs, fromJSONField: (field: any) => Clock.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Clock.fromJSON( json, ), fromIotaParsedData: (content: IotaParsedData) => Clock.fromIotaParsedData( content, ), fromIotaObjectData: (content: IotaObjectData) => Clock.fromIotaObjectData( content, ), fetch: async (client: IotaClient, id: string) => Clock.fetch( client, id, ), new: ( fields: ClockFields, ) => { return new Clock( [], fields ) }, kind: "StructClassReified", } }
