...
export function isVerifiedID(type: string): boolean { type = compressIotaType(type); return type === `${PKG_V5}::zklogin_verified_id::VerifiedID`; }
...
export class VerifiedID implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V5}::zklogin_verified_id::VerifiedID`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = VerifiedID.$typeName; readonly $fullTypeName: `${typeof PKG_V5}::zklogin_verified_id::VerifiedID`; readonly $typeArgs: []; readonly $isPhantom = VerifiedID.$isPhantom;
 ...
 private constructor(typeArgs: [], fields: VerifiedIDFields, ) { this.$fullTypeName = composeIotaType( VerifiedID.$typeName, ...typeArgs ) as `${typeof PKG_V5}::zklogin_verified_id::VerifiedID`; this.$typeArgs = typeArgs;
 ...
 static reified( ): VerifiedIDReified { return { typeName: VerifiedID.$typeName, fullTypeName: composeIotaType( VerifiedID.$typeName, ...[] ) as `${typeof PKG_V5}::zklogin_verified_id::VerifiedID`, typeArgs: [ ] as [], isPhantom: VerifiedID.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => VerifiedID.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => VerifiedID.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => VerifiedID.fromBcs( data, ), bcs: VerifiedID.bcs, fromJSONField: (field: any) => VerifiedID.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => VerifiedID.fromJSON( json, ), fromIotaParsedData: (content: IotaParsedData) => VerifiedID.fromIotaParsedData( content, ), fromIotaObjectData: (content: IotaObjectData) => VerifiedID.fromIotaObjectData( content, ), fetch: async (client: IotaClient, id: string) => VerifiedID.fetch( client, id, ), new: ( fields: VerifiedIDFields, ) => { return new VerifiedID( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromIotaParsedData( content: IotaParsedData ): VerifiedID { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isVerifiedID(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a VerifiedID object`); } return VerifiedID.fromFieldsWithTypes( content ); }

 static fromIotaObjectData( data: IotaObjectData ): VerifiedID { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isVerifiedID(data.bcs.type)) { throw new Error(`object at is not a VerifiedID object`); }

 return VerifiedID.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return VerifiedID.fromIotaParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch( client: IotaClient, id: string ): Promise<VerifiedID> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching VerifiedID object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isVerifiedID(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a VerifiedID object`); }

 return VerifiedID.fromIotaObjectData( res.data ); }
