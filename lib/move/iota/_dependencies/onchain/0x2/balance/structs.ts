...
export function isBalance(type: string): boolean { type = compressIotaType(type); return type.startsWith(`${PKG_V5}::balance::Balance` + '<'); }
...
export function isSupply(type: string): boolean { type = compressIotaType(type); return type.startsWith(`${PKG_V5}::balance::Supply` + '<'); }
