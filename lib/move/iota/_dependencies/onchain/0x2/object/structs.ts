...
export function isID(type: string): boolean { type = compressIotaType(type); return type === `${PKG_V5}::object::ID`; }
...
export class ID implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V5}::object::ID`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = ID.$typeName; readonly $fullTypeName: `${typeof PKG_V5}::object::ID`; readonly $typeArgs: []; readonly $isPhantom = ID.$isPhantom;

 readonly bytes: ToField<"address">

 private constructor(typeArgs: [], fields: IDFields, ) { this.$fullTypeName = composeIotaType( ID.$typeName, ...typeArgs ) as `${typeof PKG_V5}::object::ID`; this.$typeArgs = typeArgs;

 this.bytes = fields.bytes; }

 static reified( ): IDReified { return { typeName: ID.$typeName, fullTypeName: composeIotaType( ID.$typeName, ...[] ) as `${typeof PKG_V5}::object::ID`, typeArgs: [ ] as [], isPhantom: ID.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => ID.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => ID.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => ID.fromBcs( data, ), bcs: ID.bcs, fromJSONField: (field: any) => ID.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => ID.fromJSON( json, ), fromIotaParsedData: (content: IotaParsedData) => ID.fromIotaParsedData( content, ), fromIotaObjectData: (content: IotaObjectData) => ID.fromIotaObjectData( content, ), fetch: async (client: IotaClient, id: string) => ID.fetch( client, id, ), new: ( fields: IDFields, ) => { return new ID( [], fields ) }, kind: "StructClassReified", } }
...
export function isUID(type: string): boolean { type = compressIotaType(type); return type === `${PKG_V5}::object::UID`; }
