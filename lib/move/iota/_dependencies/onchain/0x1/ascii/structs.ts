...
export function isChar(type: string): boolean { type = compressIotaType(type); return type === `${PKG_V3}::ascii::Char`; }
...
export class Char implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V3}::ascii::Char`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Char.$typeName; readonly $fullTypeName: `${typeof PKG_V3}::ascii::Char`; readonly $typeArgs: []; readonly $isPhantom = Char.$isPhantom;

 readonly byte: ToField<"u8">

 private constructor(typeArgs: [], fields: CharFields, ) { this.$fullTypeName = composeIotaType( Char.$typeName, ...typeArgs ) as `${typeof PKG_V3}::ascii::Char`; this.$typeArgs = typeArgs;

 this.byte = fields.byte; }

 static reified( ): CharReified { return { typeName: Char.$typeName, fullTypeName: composeIotaType( Char.$typeName, ...[] ) as `${typeof PKG_V3}::ascii::Char`, typeArgs: [ ] as [], isPhantom: Char.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Char.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Char.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Char.fromBcs( data, ), bcs: Char.bcs, fromJSONField: (field: any) => Char.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Char.fromJSON( json, ), fromIotaParsedData: (content: IotaParsedData) => Char.fromIotaParsedData( content, ), fromIotaObjectData: (content: IotaObjectData) => Char.fromIotaObjectData( content, ), fetch: async (client: IotaClient, id: string) => Char.fetch( client, id, ), new: ( fields: CharFields, ) => { return new Char( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromIotaParsedData( content: IotaParsedData ): Char { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isChar(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a Char object`); } return Char.fromFieldsWithTypes( content ); }

 static fromIotaObjectData( data: IotaObjectData ): Char { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isChar(data.bcs.type)) { throw new Error(`object at is not a Char object`); }

 return Char.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return Char.fromIotaParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch( client: IotaClient, id: string ): Promise<Char> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching Char object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isChar(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a Char object`); }

 return Char.fromIotaObjectData( res.data ); }

 }

/* ============================== String =============================== */

export function isString(type: string): boolean { type = compressIotaType(type); return type === `${PKG_V3}::ascii::String`; }
...
export class String implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V3}::ascii::String`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = String.$typeName; readonly $fullTypeName: `${typeof PKG_V3}::ascii::String`; readonly $typeArgs: []; readonly $isPhantom = String.$isPhantom;

 readonly bytes: ToField<Vector<"u8">>

 private constructor(typeArgs: [], fields: StringFields, ) { this.$fullTypeName = composeIotaType( String.$typeName, ...typeArgs ) as `${typeof PKG_V3}::ascii::String`; this.$typeArgs = typeArgs;

 this.bytes = fields.bytes; }

 static reified( ): StringReified { return { typeName: String.$typeName, fullTypeName: composeIotaType( String.$typeName, ...[] ) as `${typeof PKG_V3}::ascii::String`, typeArgs: [ ] as [], isPhantom: String.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => String.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => String.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => String.fromBcs( data, ), bcs: String.bcs, fromJSONField: (field: any) => String.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => String.fromJSON( json, ), fromIotaParsedData: (content: IotaParsedData) => String.fromIotaParsedData( content, ), fromIotaObjectData: (content: IotaObjectData) => String.fromIotaObjectData( content, ), fetch: async (client: IotaClient, id: string) => String.fetch( client, id, ), new: ( fields: StringFields, ) => { return new String( [], fields ) }, kind: "StructClassReified", } }
